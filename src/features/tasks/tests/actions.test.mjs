...
test('employee cannot forge task assignee or work date', async () => {
  ...
  const api = load('src/features/dashboard/actions/tasks.ts', {
    ...
    '@/database/mongodb/models/task': { TASK_LABELS: [], TASK_PRIORITIES: ['MEDIUM'], TaskModel: { create: async value => { saved = value; return { _id: 'task' }; } } },
  ...
  for (const [key, value] of Object.entries({ title: 'Review changes', description: 'Review and verify the login flow', assignedUserId: 'another-employee', workDate: '2030-01-01', dueDate: '2026-09-23' })) form.set(key, value);
