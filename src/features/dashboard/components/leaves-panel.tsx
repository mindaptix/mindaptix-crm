    ...
    <div className="space-y-6 overflow-x-hidden px-3 py-3 sm:px-7 sm:py-6">
      ...
      <section className="space-y-6">
        ...
        {isEmployeeHistoryView ? (
          <section className="min-w-0 rounded-xl border border-slate-200 bg-white px-5 py-5 shadow-sm sm:px-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-indigo-600">Leave History</p>
                <h2 className="mt-1 text-xl font-semibold tracking-tight text-slate-950">My Requests</h2>
              </div>
              <div className="inline-flex w-fit items-center rounded-md border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-medium text-slate-600">
          ...
          >
            ...
            {canReview && !isLeadershipReviewView ? (
              <div className="mb-6 max-w-full overflow-hidden rounded-xl border border-amber-200 bg-amber-50 shadow-sm">
                <div className="grid gap-4 px-5 py-5 xl:grid-cols-[minmax(0,1fr)_minmax(280px,360px)] xl:items-center">
                  <div>
                    <div className="inline-flex items-center rounded-md border border-amber-200 bg-white px-2.5 py-1 text-xs font-semibold text-amber-700">
                      Review Queue
                    </div>
            ...
            {canReview && data.employeeSummaries.length && !isLeadershipReviewView ? (
              <div className="mb-6 max-w-full">
                <div className="mb-4 flex flex-col gap-3">
                  ...
                  <div className="inline-flex w-fit items-center rounded-md border border-sky-200 bg-sky-50 px-3 py-1 text-xs font-medium text-sky-700">
                    Usage Snapshot
                  </div>
                </div>
...
function CountPill({ tone, value }: { tone: "success" | "warning" | "danger"; value: number }) {
  ...
  return <span className={`inline-flex min-w-[48px] items-center justify-center rounded-md border px-2.5 py-1.5 text-sm font-semibold ${className}`}>{value}</span>;
}
...
function ValueBlock({
  ...
  return (
    <div className={`inline-flex min-w-[88px] flex-col rounded-md border px-3 py-2 ${className}`}>
        ...
        {leaves.map((leave) => (
          <tr className="bg-white/80 transition hover:bg-sky-50/50" key={leave.id}>
            <DashboardTableCell className="whitespace-nowrap">
              <span className={`inline-flex rounded-md px-2.5 py-1 text-xs font-semibold ${getLeaveTypeBadgeClassName(leave.leaveType)}`}>
            ...
            <DashboardTableCell className="whitespace-nowrap">
              <span className="inline-flex min-w-[52px] items-center justify-center rounded-md border border-sky-200 bg-sky-50 px-2.5 py-1.5 text-sm font-semibold text-sky-700">
                {leave.requestedDays}
              </span>
            </DashboardTableCell>
            <DashboardTableCell>
            ...
            <DashboardTableCell className="whitespace-nowrap">
              <span className={`inline-flex rounded-md px-2.5 py-1 text-xs font-semibold ${getStatusBadgeClassName(leave.status)}`}>
            ...
            <DashboardTableCell className="whitespace-nowrap">
              <form action={deleteLeaveRequest}>
                ...
                <FormActionButton
                  className="rounded-md border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 shadow-none hover:border-slate-300 hover:bg-slate-50"
                ...
                {canReview && leave.status === "PENDING" ? (
                  <>
                    <form action={reviewLeaveRequest} onSubmit={() => emitDashboardSync("leave-reviewed")}>
                      ...
                      <FormActionButton className="min-w-[94px] rounded-md px-3 py-2 text-sm shadow-none" pendingLabel="Saving..." type="submit">
                        Approve
                      </FormActionButton>
                    </form>
                    <form action={reviewLeaveRequest} onSubmit={() => emitDashboardSync("leave-reviewed")}>
                      ...
                      <FormActionButton
                        className="min-w-[94px] rounded-md border border-rose-200 bg-rose-50 px-3 py-2 text-sm font-semibold text-rose-700 shadow-none hover:border-rose-300 hover:bg-rose-100"
                  ...
                  <div className="inline-flex items-center rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-xs font-medium text-slate-500">
                    Processed
                  </div>
                )}
              </div>
  ...
  return (
    <article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-[minmax(0,1fr)_140px_160px]">
        ...
        <div className="rounded-lg border border-sky-200 bg-sky-50 px-4 py-3 text-sky-700">
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {canReview && leave.status === "PENDING" ? (
          <>
            <form action={reviewLeaveRequest} onSubmit={() => emitDashboardSync("leave-reviewed")}>
              <input name="leaveId" type="hidden" value={leave.id} />
              <input name="status" type="hidden" value="APPROVED" />
              <FormActionButton className="min-w-[120px] rounded-md px-4 py-2.5 text-sm shadow-none sm:w-auto" pendingLabel="Saving..." type="submit">
                Approve
              </FormActionButton>
            </form>
            <form action={reviewLeaveRequest} onSubmit={() => emitDashboardSync("leave-reviewed")}>
              ...
              <input name="status" type="hidden" value="REJECTED" />
              <FormActionButton
                className="min-w-[120px] rounded-md border border-rose-200 bg-rose-50 px-4 py-2.5 text-sm font-semibold text-rose-700 shadow-none hover:border-rose-300 hover:bg-rose-100 sm:w-auto"
                pendingLabel="Rejecting..."
                type="submit"
              >
                Reject
          ...
          <div className="inline-flex items-center rounded-md border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-medium text-slate-500">
        ...
        {canDelete ? (
          <form action={deleteLeaveRequest}>
            <input name="leaveId" type="hidden" value={leave.id} />
            <FormActionButton
              className="min-w-[120px] rounded-md border-slate-200 bg-white px-4 py-2.5 text-sm text-slate-700 shadow-none hover:border-slate-300 hover:bg-slate-50 sm:w-auto"
              pendingLabel="Deleting..."
              type="submit"
            >
              Delete
...
const OVERVIEW_CARD_CONFIG: Record<string, {
  ...
  APPROVED: {
    ...
    icon: (
      <svg fill="none" height="22" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.2" viewBox="0 0 24 24" width="22">
...
function OverviewCard({ detail, label, value }: { detail: string; label: string; value: string }) {
  ...
  return (
    <article className={`rounded-xl border p-5 ${config.className}`}>
...
function TextAreaField({ defaultValue, label, name, placeholder }: Omit<FieldProps, "type">) {
  return (
    <div>
      ...
      <div className="relative rounded-md border border-slate-300 bg-white transition focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-100">
