...
export async function collectGithubEvidence(repoUrl: string, username: string, branch: string, workDate: string, signal?: AbortSignal): Promise<GithubEvidence> {
  ...
  const metadata = await fetchJson(base, { headers, signal }) as { default_branch?: string };
  ...
  for (let page = 1; page <= 3; page++) {
    const rows = await fetchJson(`${base}/commits?${query}&page=${page}`, { headers, signal });
  ...
  for (let index = 0; index < selected.length; index += 5) {
    details.push(...await Promise.all(selected.slice(index, index + 5).map(async (commit) => await fetchJson(`${base}/commits/${commit.sha}`, { headers, signal }) as Commit)));
  }
  ...
  const commits: CommitEvidence[] = details.map((commit, index) => {
    ...
    if (allFiles.length === 0 || allFiles.length > 25) limited = true;
